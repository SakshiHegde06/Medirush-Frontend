...
import { saveConsultation } from "../api/index"

export default function ConsultRoom() {
  const { callDoctor } = useSocket()
  const { doctorId } = useParams()
  const { doctors, currentUser } = useApp()
  const navigate = useNavigate()
  const doctor = doctors.find(d => d.id === parseInt(doctorId) || d.id === doctorId)
  ...
  useEffect(() => {
    if (step === "consult" && mode === "chat") {
      setTimeout(() => {
        setMessages([{ role: "doctor", text: `Hello! I am ${doctor?.name}. How can I help you today?` }])
      }, 1000)
    }
  ...
  const handlePay = () => {
    ...
    setTimeout(() => {
      ...
      callDoctor({
        ...
        doctorId: doctor.id,
        roomId: `room_${doctor.id}_${Date.now()}`,
      })
    }, 2000)
  }
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      const replies = [
        "I understand your concern. Can you tell me how long you have been experiencing this?",
        "That is helpful information. Have you taken any medication for this?",
        "Based on what you have described, I would recommend some tests.",
        "This sounds manageable. I will prescribe some medication and lifestyle changes.",
        "Please follow up with me in 3 days if symptoms do not improve.",
  ...
  if (!doctor) return (
    <div className="page-wrapper">
      <Navbar />
      <div style={{ minHeight: "calc(100vh - 73px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: "var(--text-secondary)" }}>Doctor not found. <button onClick={() => navigate("/patient/consult")} style={{ background: "none", border: "none", color: "var(--teal)", cursor: "pointer" }}>Go back</button></p>
      </div>
    </div>
  )
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "40px 24px", position: "relative", zIndex: 1 }}>
        ...
        <div className="glass-card" style={{ padding: "24px", marginBottom: 28, border: "1px solid rgba(0,201,167,0.2)", display: "flex", gap: 16, alignItems: "center" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ color: "var(--text-secondary)", fontSize: "0.8rem" }}>{doctor.experience} yrs exp · ⭐ {parseFloat(doctor.rating || 0).toFixed(1)} · {(doctor.total_consultations || 0).toLocaleString()} consultations</p>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: "1.8rem", color: "var(--teal)" }}>Rs.{doctor.fee}</div>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "40px 24px", position: "relative", zIndex: 1 }}>
        ...
        <div className="glass-card" style={{ padding: "24px", marginBottom: 20, border: "1px solid rgba(0,201,167,0.2)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "1rem", fontWeight: 700 }}>
            <span>Total</span>
            <span style={{ color: "var(--teal)", fontFamily: "'DM Serif Display', serif", fontSize: "1.4rem" }}>Rs.{doctor.fee}</span>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 8 }}>
          {[["💜", "PhonePe"], ["🔵", "GPay"], ["🔷", "Paytm"]].map(([icon, name]) => (
            <button key={name} onClick={() => setSelectedUpi(name)} style={{
              ...
              <span>{name}</span>
              {selectedUpi === name && <span style={{ fontSize: "0.7rem", background: "var(--teal)", color: "var(--navy)", padding: "2px 8px", borderRadius: 100, fontWeight: 700 }}>✓ Selected</span>}
            </button>
          ))}
        </div>
        ...
        {selectedUpi && (
          <div style={{ background: "rgba(0,201,167,0.08)", border: "1px solid rgba(0,201,167,0.2)", borderRadius: 8, padding: "10px 14px", fontSize: "0.78rem", color: "var(--teal)", marginBottom: 16, marginTop: 8 }}>
            ✅ Paying via {selectedUpi} · Rs.{doctor.fee}
          </div>
        )}

        <button className="btn-primary" onClick={handlePay} disabled={paying || !selectedUpi}
          style={{ width: "100%", padding: "14px", fontSize: "0.95rem", opacity: !selectedUpi ? 0.5 : 1, marginTop: 4 }}>
          {paying ? (
            <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
              <span className="spinner" style={{ width: 18, height: 18, borderWidth: 2 }} />
              Processing via {selectedUpi}...
            </span>
          ) : selectedUpi ? `Pay Rs.${doctor.fee} via ${selectedUpi} →` : "Select a payment method first"}
        </button>
      ...
      {mode === "chat" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", maxWidth: 700, width: "100%", margin: "0 auto", padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", gap: 10, paddingTop: 16, borderTop: "1px solid var(--border)", marginTop: 16 }}>
            ...
            <button onClick={sendMessage} style={{ background: "var(--teal)", border: "none", borderRadius: 10, width: 44, height: 44, cursor: "pointer", fontSize: 18, color: "var(--navy)", fontWeight: 700 }}>→</button>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative" }}>
          {mode === "video" && (
            <div style={{ width: "100%", maxWidth: 800, height: 420, background: videoOff ? "rgba(255,255,255,0.03)" : "linear-gradient(135deg, rgba(0,201,167,0.1), rgba(0,0,0,0.5))", borderRadius: 16, border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20, position: "relative", overflow: "hidden" }}>
              {!callActive ? (
                ...
                <div style={{ textAlign: "center" }}>
                  <div style={{ width: 100, height: 100, borderRadius: "50%", background: "linear-gradient(135deg, var(--teal), var(--teal-dark))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, color: "var(--navy)", fontWeight: 700, margin: "0 auto 16px" }}>
                    {doctor.name?.split(" ")[1]?.[0] || "D"}
                  </div>
          ...
          {mode === "audio" && (
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <div style={{ width: 120, height: 120, borderRadius: "50%", background: "linear-gradient(135deg, var(--teal), var(--teal-dark))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 48, color: "var(--navy)", fontWeight: 700, margin: "0 auto 20px" }}>
                {doctor.name?.split(" ")[1]?.[0] || "D"}
              </div>
          ...
          {callActive && (
            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
              <button onClick={() => setMuted(!muted)} style={{ width: 56, height: 56, borderRadius: "50%", border: "none", background: muted ? "rgba(255,107,107,0.2)" : "rgba(255,255,255,0.1)", color: muted ? "var(--coral)" : "var(--text-primary)", cursor: "pointer", fontSize: 22, transition: "all 0.2s" }}>
                {muted ? "🔇" : "🎤"}
              </button>
              {mode === "video" && (
                <button onClick={() => setVideoOff(!videoOff)} style={{ width: 56, height: 56, borderRadius: "50%", border: "none", background: videoOff ? "rgba(255,107,107,0.2)" : "rgba(255,255,255,0.1)", color: videoOff ? "var(--coral)" : "var(--text-primary)", cursor: "pointer", fontSize: 22, transition: "all 0.2s" }}>
                  {videoOff ? "📷" : "🎥"}
                </button>
              )}
              <button onClick={handleEndCall} style={{ width: 64, height: 64, borderRadius: "50%", border: "none", background: "var(--coral)", color: "white", cursor: "pointer", fontSize: 24, transition: "all 0.2s", boxShadow: "0 4px 20px rgba(255,107,107,0.4)" }}>
                📵
              </button>
              <button style={{ width: 56, height: 56, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.1)", color: "var(--text-primary)", cursor: "pointer", fontSize: 22 }}>
                🔊
              </button>
            </div>
          )}
